...
| 开发阶段 | **Phase 2.x — Step 1 (data_dispatcher) 已完成** |
| 当前目标 | PyQt 透明窗口 + 弹幕滚动渲染 |
| 下一目标 | Step 2: PyQt Overlay 层 (main_window + tray_icon + renderer) |
...
| 工作区 | **有未提交修改** — v0.2.0-alpha 实机验证修复 + v0.2.x Step 1 (见 §5) |
...
- [ ] Step 2: PyQt Overlay 层 (main_window + tray_icon + renderer + danmaku_queue)
- [ ] Step 3: 重构 native_host.py (stdin→后台线程 + dispatcher 集成)
- [ ] Step 4: 端到端串联测试
...
以下文件在 v0.2.0-alpha 实机验证 + v0.2.x Step 1 中有修改，尚未提交：
...
## 6. 下一步：v0.2.x Step 2（PyQt Overlay 基础窗口）
...
- **零依赖**：纯 Python 标准库（`threading.Lock` + `queue.Queue` 预留），不依赖 PyQt

### 6.2 Step 2 目标

创建 PyQt6 Overlay 基础窗口层，不修改 native_host.py，不串联弹幕数据。

**需新建的文件：**

| # | 文件 | 职责 |
|---|------|------|
| 1 | `python/overlay/__init__.py` | 包标记 |
| 2 | `python/overlay/main_window.py` | 透明无边框置顶窗口 + 鼠标穿透（`WS_EX_TRANSPARENT`） |
| 3 | `python/overlay/tray_icon.py` | 系统托盘 + 右键退出 |
| 4 | `python/overlay/danmaku_renderer.py` | QPainter 滚动弹幕 + 多轨道管理 |
| 5 | `python/overlay/danmaku_queue.py` | 弹幕缓冲队列 + 线程安全 + 墙上时钟发射 |
...
### 6.3 完整 v0.2.x 实施计划（共 4 步）

```
Step 1 ✅  Step 2        Step 3           Step 4
──────▶    ──────▶       ──────▶          ──────▶
events +   PyQt Overlay  native_host      端到端串联
dispatcher 基础窗口层    重构+集成         测试验证
```

| Step | 内容 | 新建 | 修改 |
|------|------|------|------|
| 1 | events + data_dispatcher | 2 文件 | 0 |
| 2 | overlay 基础窗口层 | 5 文件 | requirements.txt |
