...
│   ├── events.py                   # [v0.2.x Step1] 事件类型枚举 + 数据类 (4 事件类型: VIDEO_SWITCHED/DANMAKU_LOADED/PROGRESS_UPDATED/VIDEO_UNLOAD)
...
└── tests/
    ...
    ├── test_dispatcher.py         # [v0.2.x Step1] dispatcher + events 单测（52/52 通过）
    ...
    ├── test_integration.py         # [v0.2.x Step3] 集成测试：Integration + paintEvent + cid + video_unload（23/23 通过）
    └── mock/
...
| 工作区 | **有未提交修改** — Issue 5 video_unload 生命周期修复 (Round 1 + Round 2 window hide/show) (见 §5) |
...
```
Extension (Browser)
  │  content.js — Resolver Chain + 播放监听 + Cookie采集 + pagehide→video_unload
  │  background.js — 消息路由 + Native Messaging 连接 + 惰性连接 + 状态持久化 + 重连退避
  ↓
...
| `tests/test_dispatcher.py` | **52/52** | 事件数据类(15) + 订阅(5) + 发布(7) + 取消(4) + 计数(4) + Reset(2) + 异常隔离(2) + 线程安全(3) + 类型推断(6) + 单例(2) + 模块级清理(2) |
...
| `tests/test_integration.py` | **23/23** | Integration(10) + dispatcher隔离(1) + paintEvent(2) + cid契约(3) + video_switch清空(1) + failure清除renderer(1) + failure清除queue+停clock(1) + video_unload清除(3) + video_unload窗口显隐(2) + video_unload订阅(1) + quit信号(1) + 订阅注册(1) |
...
| **关闭 B站 tab 弹幕继续滚动** | Step 4 实机验证 | pagehide → video_unload 全链路清理 + window hide/show 基于 `_wall_clock_start` | 262/262, 实机验证通过 ✅ |
| **关闭 B站 tab 后窗口不消失 (Round 2)** | Issue 5 用户反馈 | `_on_frame` 中基于 `_wall_clock_start` 管理 window.hide()/show() | 262/262, 实机验证通过 ✅ |

---

## 5. 当前未提交修改

以下文件为 Issue 5 video_unload 生命周期修复 (Round 1 + Round 2)，尚未提交：

| 文件 | 修改内容 |
|------|----------|
| `docs/status.md` | Issue 5 最终验证结果 + Issue 6 后续计划（本文件） |
| `python/native_host.py` | **Issue 5 Round 1** — +`_handle_video_unload_message()` + `DanmakuIntegration._on_video_unload()` + `VIDEO_UNLOAD` 订阅；**Issue 5 Round 2** — `_on_frame()` 中基于 `_wall_clock_start` 管理 window hide()/show() |
| `python/overlay/danmaku_renderer.py` | **Issue 2 诊断** — +`frame_count` 属性、+`_on_frame` try/except 保护、+30s heartbeat stderr 日志 |
| `python/events.py` | **Issue 5** — +`VIDEO_UNLOAD` EventType + `VideoUnloadEvent` 数据类 |
| `tests/test_integration.py` | **Issue 5** — +6 测试: video_unload 清除 / 空状态安全 / 多轮切换后清除 / window hide→show cycle / 订阅确认 (23/23) |
| `tests/test_dispatcher.py` | **Issue 5** — +VideoUnloadEvent 存在性测试 + 类型推断测试 + 事件类型枚举数更新 3→4 (52/52) |
| `extension/background.js` | **Issue 3** — `chrome.storage.local` 持久化 `lastBv` + `nativePortJustConnected`；**Issue 4** — 惰性连接 + `MAX_RECONNECT_ATTEMPTS=5`；**Issue 5** — +`handleVideoUnload()` + 清理 `lastBv` |
| `extension/content.js` | **Issue 5** — +`pagehide` 监听器发送 `video_unload` + 清理 monitor/navigator |
| `extension/manifest.json` | **Issue 3** — 新增 `storage` 权限 |
| `CLAUDE.md` | 升级: +关键架构约束 +生命周期设计原则 +Debug流程 +测试规范 +AI修改行为规范 +项目状态参考 |
...
| 2 | **SW 重启误触发 video_switch** | Service Worker 终止后 `lastBv = null`，恢复后首次消息被误判为视频切换 | `chrome.storage.local` 持久化 `lastBv`（30min 会话窗口）；`nativePortJustConnected` 标志区分"Python 新进程"与"同会话恢复" | `manifest.json`, `background.js` |
| 3 | danmaku_fetcher 偶发 HTTP 412 | B站风控机制 | **记录为已知风险**（v0.3+ 增强 Cookie 采集） | — |

**v0.3 功能需求（不修，仅记录）:**

| # | 需求 | 现状 | v0.3 方案 |
|---|------|------|-----------|
| R1 | **播放/暂停同步** | v0.2 墙上时钟驱动，`progress_update` 事件未接入队列。暂停后弹幕继续滚动 | progress_update → queue 进度同步模式 | |
...
| **Issue 2: Renderer 偶尔冻结** | **诊断中** | 已添加 renderer heartbeat + shutdown lifecycle 日志；待实机复现确认根因 |

---

## 7. v0.2.x 诊断中问题

### 7.1 Issue 2: Renderer 偶尔冻结（诊断中 🔍）

**症状:** 反复切换视频/暂停后，弹幕停留在屏幕右侧不移动；关闭 B站页面后弹幕仍停留；Python 进程可能仍存在。

**已添加诊断 (2026-07-17):**

| 文件 | 变更 | 目的 |
|------|------|------|
| `python/overlay/danmaku_renderer.py` | +`frame_count` 属性、+`_on_frame` try/except 保护、+30s heartbeat stderr 日志 | 判断 QTimer/frame 是否停滞 |
| `python/native_host.py` | +shutdown 生命周期日志（stdin→quit_requested→app.exec→window.hide→renderer.stop）、+`window.hide()` 在 cleanup 中优先执行 | 判断退出链路是否异常；修复窗口残留 |

**复现时检查 `native_host.log`：**
1. `[renderer heartbeat]` — frame_count 是否停止增长？→ QTimer 已停
2. `[lifecycle]` — 是否缺少某条日志？→ 退出链在哪个环节卡住
3. Python 进程是否仍存在？→ 任务管理器检查

**待确认根因后决定是否加入 watchdog。**

---

## 8. v0.2.x 生命周期修复 (2026-07-17)

### 8.1 Issue 4: Native Host 后台驻留 + 无限重启

**症状:**
1. Python native_host 进程在没有 B站页面时出现在后台
2. 用户手动关闭后，有概率自动重启
3. 进程长期空闲运行（1h+ heartbeat, active=0, 零消息）

**根因:**

| 问题 | 代码位置 | 根因 |
|------|----------|------|
| SW 启动即启动 Python | `background.js:250` (旧) | `connectNative()` 在 SW 顶层无条件调用 |
| 手动关闭后自动重启 | `background.js:51,57` | `onDisconnect` → `attemptReconnect()` 无最大重试限制 |
| 无法区分主动/被动断开 | `background.js:44-52` | `onDisconnect` 不区分"用户关闭"与"崩溃" |

**修复 (3 处改动):**

| Fix | 文件 | 变更 |
|-----|------|------|
| **惰性连接** | `extension/background.js` | 移除 SW 顶层 `connectNative()`；`routeMessage()` 中检测 `!nativePort` 时按需调用 `connectNative('message')` |
| **限制重连** | `extension/background.js` | 新增 `MAX_RECONNECT_ATTEMPTS = 5`；`attemptReconnect()` 达上限后停止并等待新用户操作 |
| **诊断日志** | `extension/background.js` + `python/native_host.py` | `connectNative()` 记录原因/时间；`onDisconnect` 记录 lastError + lastBv；`attemptReconnect` 记录次数 + hasBilibiliSession；Python 记录 PID + 启动时间 + idle 时长 |

**修复后期望行为:**

| 场景 | 修复前 | 修复后 |
|------|--------|--------|
| SW 启动，无 B站页面 | Python 启动 | Python **不启动** (惰性连接) |
| 打开 B站视频 | video_switch 正常 | `routeMessage` → `connectNative('message')` → Python 启动 → 正常 |
| 关闭 B站页面 | Python 可能残留 | stdin 断开 → quit → Python 退出 |
| 用户手动结束 Python | `onDisconnect` → 无限重连 → Python 复活 | 最多重连 5 次 → 停止，等待新 B站页面触发惰性连接 |

### 8.2 Issue 5: 关闭 B站 tab 后弹幕继续滚动 ✅ (2026-07-17)

**目标:** 关闭 Bilibili 视频页面后，Overlay 弹幕应立即停止显示，窗口隐藏；重新打开视频后恢复正常。

**症状 (修复前):**
1. 关闭 B站视频 tab → 弹幕继续滚动
2. Python 进程继续保持运行
3. renderer heartbeat 正常 (frame 持续增长, active > 0)

**根因:** 关闭 tab 后 `content.js` 随页面销毁，但没有任何清理信号到达 Python 渲染管线。

---

**Round 1 修复 — video_unload 消息链 (5 文件):**

| 文件 | 变更 |
|------|------|
| `extension/content.js` | +`pagehide` 监听器 — 页面卸载时发送 `{type: "video_unload"}` + 清理 monitor/navigator |
| `extension/background.js` | +`handleVideoUnload()` — 转发 `video_unload` 到 Python + 清除 `lastBv` + 持久化 |
| `python/events.py` | +`EventType.VIDEO_UNLOAD` + `VideoUnloadEvent` 数据类 |
| `python/native_host.py` | +`_handle_video_unload_message()` + `DanmakuIntegration._on_video_unload()` — 清理 queue + renderer + 停止时钟 |
| `tests/test_integration.py` + `tests/test_dispatcher.py` | +6 测试: 清除/空状态/多轮/订阅/类型推断/枚举数 |

**Round 1 验证结果:** video_unload 链路正常工作（日志确认消息到达），`_on_video_unload` 成功清空 queue 和 renderer 内容。但**透明窗口本身未隐藏** — 用户看到空白的透明置顶窗口悬挂在屏幕上。

---

**Round 2 修复 — 窗口可见性管理:**

**问题:** `_on_video_unload` 清空了弹幕内容（`_pending_clear` → `renderer.clear()`，`queue.clear()`，`_wall_clock_start = None`），但没有调用 `window.hide()`。QTimer 继续 running，透明全屏窗口继续存在。

**根因:** `window.hide()` 必须在 GUI 线程调用，但 `_on_video_unload` 在后台线程执行。且不应新增专用 flag。

**修复方案:** 在 `DanmakuIntegration._on_frame()`（GUI 线程）中，利用已有的 `_wall_clock_start` 状态驱动窗口可见性：

```python
# _on_frame() 中新增逻辑（GUI 线程，每帧执行）:
if self._wall_clock_start is None:
    if self._window.isVisible():
        self._window.hide()    # 无活跃视频 → 隐藏窗口
    return

if not self._window.isVisible():
    self._window.show()         # 有活跃视频 → 显示窗口
```

**设计要点:**
- 不需要新 flag、新 signal — 纯利用 `_wall_clock_start` 现有状态
- `window.hide()`/`window.show()` 始终在 GUI 线程执行
- 重新打开 B站视频时 `_on_frame` 自动 `window.show()`（延迟 < 33ms）
...
```
关闭 B站 tab
  → pagehide 事件 → content.js 发送 video_unload
  → background.js 转发到 Python + 清除 lastBv
  → native_host.py → dispatcher.publish(VideoUnloadEvent)
  → DanmakuIntegration._on_video_unload() [后台线程]:
       _pending_clear = True       → 下一帧 renderer.clear()
       _queue.clear()              → 队列清空
       _wall_clock_start = None    → 停止发射
  → 下一帧 _on_frame() [GUI线程]:
       renderer.clear()            → 弹幕消失
       window.hide()               → 窗口隐藏 ✅ (Round 2 新增)

重新打开 B站视频
  → video_switch → DanmakuLoadedEvent → _wall_clock_start = now
  → _on_frame(): window.show() → 弹幕恢复 ✅
```
...
| 类别 | 结果 | 说明 |
|------|:--:|------|
| 自动化测试 | **262/262** ✅ | 含 `test_video_unload_window_hide_show_cycle` (完整 hide→show 循环) |
| 关闭 B站 tab | ✅ | 弹幕立即消失，窗口隐藏 |
| 重新打开 B站视频 | ✅ | 弹幕恢复，窗口显示 |
| 多次循环测试 | ✅ | video_switch 正常、视频切换正常、video_unload 正常 |
| 无需关闭 Chrome | ✅ | 单 tab 关闭即清理 |

**期望时序:**

| 事件 | 延迟 |
|------|------|
| renderer.clear() | < 33ms (下一帧) |
| window.hide() | < 33ms (下一帧) |
| 重新打开视频 window.show() | < 33ms (_wall_clock_start 设置后下一帧) |
| Python 进程退出 | SW 空闲挂起后 (~30s, Chrome 行为) |

---

### 8.3 已知限制 / 后续 Issue

**Issue 6: Native Host idle 生命周期优化 (待规划)**

**当前行为:** video_unload 后：
- ✅ Overlay 已隐藏
- ✅ renderer 已清空
- ❌ Native Host Python 进程仍保持运行

**原因:** Native Host 生命周期目前与 Chrome extension connection 绑定。`video_unload` 只代表当前视频结束，不代表 extension disconnect。Python 进程继续等待下一条消息。

**影响:**
- 用户关闭所有 B站 tab 后，Python 进程在后台占用资源
- 进程仅在 Chrome 关闭或 SW 空闲挂起后才退出（~30s+）

**后续方案 (v0.3+):**
- 无 Bilibili 视频活动时，根据策略决定是否退出 Native Host
- 可能方案: idle 超时自动退出、监听 SW disconnect、或保持长连接但降低资源占用

---

### 8.4 待解决：HTTP 412 danmaku_fetcher 风控

**状态:** 分析完成，暂不修改。

**根因:** Python `urllib` 请求头与真实浏览器差异过大，B站 openresty WAF 拦截老 XML 弹幕 API (`/x/v1/dm/list.so`)。缺失 `Accept-Language`、`Accept-Encoding` 头。

**待修复方案:** 补全 `constants.py` `HTTP_HEADERS` 增加 `Accept-Language` + `Accept-Encoding` + `Cache-Control`。
