...
> 最后更新：2026-07-15 (v0.2.x Step 3 完成 — dispatcher 集成)
...
| `tests/test_integration.py` | **17/17** 🆕 | Integration(8) + dispatcher隔离(1) + paintEvent(2) + cid契约(3) + video_switch清空(1) + 订阅注册(1) + quit信号(1) |
...
| `python/native_host.py` | **重构** — QApplication 入口 + 后台 stdin daemon 线程 + DanmakuIntegration 协调器（Step 3） |
| `python/overlay/main_window.py` | **修改** — 新增 paintEvent() 调用 render 回调（Step 3） |
| `python/overlay/__init__.py` | **修改** — 更新 docstring 反映当前 4 个模块（Step 3） |
| `python/danmaku_handler.py` | **修改** — 返回值新增 `cid` 字段（Step 3） |
| `tests/test_danmaku_renderer.py` | **新建** — danmaku_renderer 单测（75/75 通过） |
| `tests/test_integration.py` | **新建** — Step 3 集成测试（17/17 通过） |
