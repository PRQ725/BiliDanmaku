...
| 开发阶段 | **Phase 2.x — Step 2.1 (PyQt Overlay 基础层) 已完成** |
| 当前目标 | PyQt 透明窗口已完成 → 弹幕渲染引擎 + 缓冲队列 |
| 下一目标 | Step 2.2: danmaku_renderer + danmaku_queue |
...
│   ├── overlay/                    # [v0.2.x Step2.1] PyQt 桌面覆盖层
│   │   ├── __init__.py
...
| 工作区 | **有未提交修改** — v0.2.0-alpha 实机验证修复 + v0.2.x Step 1 + Step 2.1 (见 §5) |
...
**Phase 2.x (v0.2.x) — Step 1 + Step 2.1 已完成 ✅**
...
- [ ] Step 2.2: danmaku_renderer + danmaku_queue (弹幕渲染引擎 + 缓冲队列)
...
以下文件在 v0.2.0-alpha 实机验证 + v0.2.x Step 1 + Step 2.1 中有修改，尚未提交：
...
## 6. 下一步：v0.2.x Step 2.2（弹幕渲染引擎 + 缓冲队列）
...
创建弹幕渲染引擎和缓冲队列，实现弹幕从队列到屏幕的完整绘制管线。
...
| 2 | `python/overlay/danmaku_queue.py` | 弹幕缓冲队列 + 线程安全 + 墙上时钟发射 |

**Step 2.2 不修改的文件：**
- `python/native_host.py`（Step 3 再重构）
- `python/data_dispatcher.py`（Step 3 再接入）

### 6.4 完整 v0.2.x 实施计划（共 4 步）

```
Step 1 ✅     Step 2.1 ✅    Step 2.2        Step 3           Step 4
──────▶       ──────▶        ──────▶         ──────▶          ──────▶
events +      PyQt Overlay   danmaku_        native_host      端到端串联
dispatcher    基础窗口层     renderer+queue  重构+集成         测试验证
```
...
| 2.2 | danmaku_renderer + danmaku_queue | 2 文件 | 0 |
