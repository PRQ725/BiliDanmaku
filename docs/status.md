...
└── tests/
    ...
    ├── test_danmaku_queue.py       # [v0.2.x Step2.2a] danmaku_queue 单测（51/51 通过）
    ├── test_danmaku_renderer.py   # [v0.2.x Step2.2b] danmaku_renderer 单测（75/75 通过）
    ├── test_integration.py         # [v0.2.x Step3] 集成测试：Integration + paintEvent + cid + video_unload（24/24 通过）
    └── mock/
...
|------|------|
| 当前分支 | `main` |
| 最新提交 | `4593ca6` — Fix native host lifecycle cleanup flow |
| 标签 | `v0.1.0` → `v0.1.1` → `v0.1.2` → `v0.2.0-alpha` (当前) |
| 工作区 | clean — 所有 Issue 5 变更已提交 |
...
- [x] `tests/test_dispatcher.py` — 52/52 通过 (事件数据类 + 订阅/发布/取消 + 线程安全 + 异常隔离 + 类型推断)
...
- [x] `tests/test_danmaku_queue.py` — 51/51 通过 (构造 + load + tick + clear + 属性 + 线程安全 + 边界)
...
| `tests/test_danmaku_queue.py` | **51/51** | 构造(5) + load(7) + tick(14) + clear(5) + 属性(6) + 线程安全(3) + 边界(11) |
| `tests/test_integration.py` | **24/24** | Integration + dispatcher隔离 + paintEvent + cid契约 + video_switch清空 + failure清除 + video_unload清除 + video_unload窗口显隐 + quit信号 + 订阅注册 |
...
以下为 v0.2.x 生命周期修复阶段（2026-07-17）已提交的变更：

| 提交 | 内容 |
|------|------|
| `4593ca6` | **native_host lifecycle cleanup flow** — Issue 5 Round 1+2: video_unload 全链路清理 + window hide/show |
| `a7c7318` | **Fix video switch cleanup and SW persistence** — Issue 1 (旧弹幕残留) + Issue 3 (SW 持久化) |
| `7846c1e` | **Qt overlay integration** — native_host 重构 + DanmakuIntegration 协调器 |
| `7501b7e` | **Overlay rendering pipeline** — danmaku_renderer + danmaku_queue |
| `2589505` | **Danmaku buffering pipeline** — Step 2.2a 弹幕缓冲队列 |

涉及文件：`python/native_host.py`, `python/events.py`, `python/overlay/danmaku_renderer.py`, `tests/test_integration.py`, `tests/test_dispatcher.py`, `extension/background.js`, `extension/content.js`, `extension/manifest.json`, `docs/status.md`, `CLAUDE.md`
...
| `tests/test_danmaku_queue.py` | ~430 | 51 项单测覆盖：构造 / load / tick / clear / 属性 / 线程安全 / 边界 |
...
### 6.7 Step 4 产出 — 端到端实机验证 + Bug 修复（已完成 ✅）
