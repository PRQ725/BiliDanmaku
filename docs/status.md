...
> 最后更新：2026-07-15 (v0.2.x Overlay 基础设施完成)
...
| 开发阶段 | **Phase 2.x — v0.2.x Overlay 基础设施已完成 (Step 1–2.2b)** |
| 当前目标 | 弹幕渲染引擎已完成 → 重构 native_host 接入 dispatcher |
| 下一目标 | Step 3: native_host.py 重构 + dispatcher 集成 |
...
- [ ] Step 3: native_host 重构 + dispatcher 集成
...
Native Messaging (stdin/stdout, JSON + 4字节LE长度前缀)
  ↓
native_host.py — 消息入口 + 协议编解码 + Cookie提取
  ↓
...
v0.2.x 已实现、尚未与 native_host 串联 (Step 3):
  ...
  ↓
  main_window — 透明无边框置顶窗口 + 鼠标穿透 + 全屏覆盖
  tray_icon — 系统托盘 + 右键退出菜单
```

**核心原则：扩展是"眼睛"（观察上报），Python 是"大脑和手"（决策执行）。**

**当前架构状态：** data_dispatcher + overlay 层（main_window / tray_icon / danmaku_queue / danmaku_renderer）已全部实现。native_host.py → danmaku_handler 弹幕获取链路已完成并实机验证。两者尚未串联（Step 3）。
...
以下文件在 v0.2.0-alpha 实机验证 + v0.2.x Step 1 + Step 2.1 + Step 2.2a + Step 2.2b 中有修改，尚未提交：
...
| `python/overlay/danmaku_renderer.py` | **新建** — QPainter 弹幕渲染引擎 + 多轨道管理 + QTimer 帧驱动 |
| `tests/test_danmaku_renderer.py` | **新建** — danmaku_renderer 单测（75/75 通过） |
...
**Step 3 将修改的文件：**
- `python/native_host.py` — stdin 移到后台线程 + dispatcher 集成
- 可能微量修改 `danmaku_handler.py`（适配 dispatcher 接口）

**Step 3 不修改的文件：**
- `python/overlay/` 下所有模块
